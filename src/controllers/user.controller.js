...
const getUser = catchAsync(async (req, res) => {
  ...
  if (req.user.email !== data.email) {
    throw new ApiError(httpStatus.FORBIDDEN, "Use your authentication");
  }
  if (data === null || data === undefined) {
    throw new ApiError(httpStatus.NOT_FOUND, "User Not Found");
  }
  if (req.query.q == "address") {
    const data = await userService.getUserAddressById(userId);
...
const setAddress = catchAsync(async (req, res) => {
  ...
  if (!req.body.address || req.body.address.length < 20) {
    throw new ApiError(
      ...
      "Address cannot be empty or less than 20 characters"
    );
  }
